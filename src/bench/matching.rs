...
pub(crate) fn best_benchmark_match<'a>(quote: &Quote, benchmarks: &'a [Benchmark]) -> Option<&'a Benchmark> {
    ...
    let q_creator = canonical_creator(if quote.creator.is_empty() { &inferred } else { &quote.creator });
    ...
    for b in benchmarks {
        ...
        let mut score = token_jaccard(&q_name, &b_key_name)
            .max(token_jaccard(&q_model, &b_key_slug));
...
pub(crate) fn keys_differ_only_by_release_tag(a: &str, b: &str) -> bool {
    ...
    !short_tokens.is_empty()
        ...
        && long_tokens.difference(&short_tokens).all(|token| {
...
pub(crate) fn canonical_creator(s: &str) -> String {
    let n = normalize(s);
    if n == "z ai" || n == "zai" || n.contains("zhipu") { "zai".into() }
    else if n == "open ai" || n == "openai" { "openai".into() }
    else if n == "x ai" || n == "xai" { "xai".into() }
    else if n.contains("anthropic") { "anthropic".into() }
    else if n.contains("alibaba") { "alibaba".into() }
    else if n.contains("moonshot") { "moonshot".into() }
    else if n.contains("deepseek") { "deepseek".into() }
    else if n.contains("minimax") { "minimax".into() }
    else if n.contains("xiaomi") { "xiaomi".into() }
    else if n.contains("tencent") { "tencent".into() }
    else if n.contains("mistral") { "mistral".into() }
    else if n.contains("google") || n.contains("deepmind") { "google".into() }
    else { n.replace(" ai", "") }
}
...
pub(crate) fn model_core(s: &str) -> String {
    let tokens = normalize(s).split_whitespace().map(str::to_owned).collect::<Vec<_>>();
    let mut out = Vec::new();
...
pub(crate) fn benchmark_model_key(s: &str) -> String {
    ...
    let tokens = core.split_whitespace().map(str::to_owned).collect::<Vec<_>>();
    ...
    if tokens.len() >= 3 && tokens[0] == "open" && tokens[1] == "ai" {
        ...
        && matches!(tokens[0].as_str(), "openai" | "anthropic" | "google" | "xai" | "alibaba" | "moonshot" | "meta" | "zhipu" | "xiaomi" | "nvidia")
    {
        tokens.remove(0);
    }
    ...
    while tokens.first().is_some_and(|token| matches!(token.as_str(), "e2ee" | "web" | "private")) {
        tokens.remove(0);
    }
    ...
    if tokens.first().is_some_and(|token| token == "claude") {
        ...
        if let Some(pos) = tokens.iter().position(|token| {
            matches!(token.as_str(), "opus" | "sonnet" | "haiku" | "fable")
        }) {
            if pos != 0 {
...
pub(crate) fn is_year_token(token: &str) -> bool {
    token.len() == 4
        ...
        && token.parse::<u32>().map(|year| (2024..=2035).contains(&year)).unwrap_or(false)
}

pub(crate) fn is_model_modifier(token: &str) -> bool {
    matches!(
        token,
        "auto" | "low" | "medium" | "high" | "xhigh" | "effort" |
        "max" | "preview" | "latest" | "basic" | "minimal" | "standard" |
        "web" | "e2ee" | "private" | "reasoning" | "nonreasoning" |
        "fast" | "highspeed" | "instruct" | "it"
    ) || (token.ends_with('k') && token[..token.len().saturating_sub(1)].chars().all(|c| c.is_ascii_digit()))
}
...
mod tests {
    ...
    fn livebench_effort_suffix_matches_base_model() {
        ...
        let b = vec![Benchmark {
            slug: "gpt-5.5-xhigh".into(), name: "gpt-5.5-xhigh".into(), creator: "OpenAI".into(),
            overall: Some(90.0), coding: Some(82.0), agentic_coding: Some(70.0),
            agent: None, reasoning_effort: None, kind: BenchmarkKind::Model,
    ...
    fn deployment_wrappers_inherit_base_model_benchmark_key() {
        assert_eq!(benchmark_model_key("e2ee-glm-5"), benchmark_model_key("GLM-5"));
        assert_eq!(benchmark_model_key("deepseek-v4-flash:web"), benchmark_model_key("DeepSeek V4 Flash"));
    }
    ...
    fn benchmark_aliases_match_real_cross_source_names() {
        for (surplus, benchmark) in [
            ...
            let b = vec![score_benchmark(benchmark, 50.0, None, None, BenchmarkKind::Model)];
            assert!(best_benchmark_match(&q, &b).is_some(), "{surplus} should match {benchmark}");
        }
    }
