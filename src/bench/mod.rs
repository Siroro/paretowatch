...
use crate::types::{
    Benchmark, BenchmarkKind, BenchmarkSource, ComparisonMode, default_common_scaffold,
};

pub(crate) fn benchmark_row_allowed(
    row: &Benchmark,
    mode: ComparisonMode,
    common_scaffold: &str,
) -> bool {
...
pub(crate) fn available_scaffolds(sets: &HashMap<BenchmarkSource, Vec<Benchmark>>) -> Vec<String> {
    ...
    out.extend(rest);
    if out.is_empty() {
        out.push(default_common_scaffold());
    }
    out
}
...
pub(crate) fn collapse_benchmark_rows(
    ...
    for row in rows
        .iter()
        .filter(|row| benchmark_row_allowed(row, mode, common_scaffold))
    {
...
pub(crate) fn benchmarks_for_source(
    ...
    let Some(rows) = sets.get(&source) else {
        return Vec::new();
    };
    ...
    // Some useful leaderboards (notably Revelo Code Index) only publish
    // model+agent rows. In "Model capability" mode, do not make
...
mod tests {
    ...
    fn common_scaffold_mode_selects_only_matching_harness() {
        ...
        let collapsed =
            collapse_benchmark_rows(&rows, ComparisonMode::CommonScaffold, "mini-SWE-agent");
